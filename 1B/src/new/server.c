...
int main(int argc, char *argv[])
{
	...
	if(( res = bind( sockfd, aip->ai_addr, aip->ai_addrlen )) == -1) {
		close(sockfd);
		perror("server: bind");
	}
	...
	if(( connfd = accept( sockfd, (struct sockaddr *)&their_addr, &addr_size)) == -1)
	{
		perror( "Could not accept connection" );
	}
	
	//ready to communicate on socket descriptor connfd
	...
	for (round = 1; round <= MAX_TRIES; ++round) {
		...
		// << verschieben nach links, um den jeweils dahinter angegebenen Wert
		// | bitweise Oder-Verknüpfung, verknüpft jedes Bit der beiden Operatoren
		...
		/* send message to client, just copy and invert read_from_client */
	/*	int bytes_sent = 0;
		do {
			int s;
			 On success send returns the number of characters sent. 
			if(( s = send(connfd, &buffer[0] + bytes_sent, s - bytes_sent, 0)) == -1)
			{
				bail_out(EXIT_FAILURE, "send");
			}
			fprintf(stdout, "SENT BUFFER: 0x%x\n", buffer[0]);
